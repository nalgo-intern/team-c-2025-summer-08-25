...
from oni_double_env import ChaseEnv # 強化学習環境をインポート
...
rope = None
oni_positions = [] # ▼▼▼【変更】鬼の座標リストを管理 ▼▼▼
MODE = "START_SCREEN"

# --- 鬼の移動ロジックのヘルパー関数 ---
def apply_oni_action(oni_pos, action):
    """
    強化学習モデルから得られた行動を、座標の移動に変換する。
    """
    next_pos = oni_pos.copy()
    if action == 0: next_pos[1] -= 1  # 上
    if action == 1: next_pos[1] += 1  # 下
    if action == 2: next_pos[0] -= 1  # 左
    if action == 3: next_pos[0] += 1  # 右

    # 岩や壁にぶつかる場合は移動しない
    if (0 <= next_pos[0] < GRID_WIDTH and 0 <= next_pos[1] < GRID_HEIGHT and
            field[next_pos[1]][next_pos[0]]["type"] != "rock"):
        return next_pos
    else:
        return oni_pos
...
def reset_game():
    global player_x, player_y, field, last_move_time, coin_count, rope, oni_positions
    player_x, player_y = rope_pos; coin_count = 0; rope = None
    oni_positions = [] # 鬼の座標リストをリセット
    ...
    for _ in range(2):
        while True:
            ...
            if (ox, oy) not in occupied_positions and distance >= 3:
                oni_positions.append(np.array([ox, oy]))
...
ONI_MOVE_INTERVAL = 300 # 鬼の移動間隔を固定
last_oni_move_time = 0 # 鬼の最終移動時間を追跡
...
while True:
    ...
    if MODE == "START_SCREEN":
        ...
        # 鬼がプレイヤーを捕まえたか判定
        if np.array_equal(oni_positions[0], np.array([player_x, player_y])) or np.array_equal(oni_positions[1], np.array([player_x, player_y])):
            MODE = "GAME_OVER"
        # ▲▲▲【変更】ここまで ▲▲▲
        ...
        for oni_pos in oni_positions:
            screen.blit(oni_img, (oni_pos[0] * GRID_SIZE, oni_pos[1] * GRID_SIZE))
        # ▲▲▲【変更】ここまで ▲▲▲
