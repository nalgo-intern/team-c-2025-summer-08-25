...
# ▼▼▼【変更】ゲームオーバー用のフォントを準備 ▼▼▼
game_over_font = pygame.font.Font(None, 100) # フォントとサイズを指定
...
while True: # game_overフラグでの終了は最後に行う
            ...
            for y in range(GRID_HEIGHT):
                ...
                for x in range(GRID_WIDTH):
                    if field[y][x]["type"] == "rock":
                        ...
                        row_str += "."
                bfs_map.append(row_str)

            oni_next_position = get_oni_next_move(bfs_map, (oni_x, oni_y), (player_x, player_y))
    ...
    if game_over:
        ...
        pygame.display.flip() # 画面を更新して文字を表示
